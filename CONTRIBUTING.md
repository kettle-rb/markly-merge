...
1. - One spec file per class/module. For each class or module under `lib/`, keep all of its unit tests in a single spec file under `spec/` that mirrors the path and file name exactly: `lib/markly/merge/my_class.rb` -> `spec/markly/merge/my_class_spec.rb`.
