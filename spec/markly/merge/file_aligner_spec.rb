  ...
  describe "match refiner integration" do
    ...
    context "when refiner matches already-matched nodes" do
      ...
      it "does not duplicate matches" do
        ...
        heading_matches = matches.select do |m|
          node = m[:template_node]
          # Check merge_type on typed nodes, or check type string
          (node.respond_to?(:merge_type) && node.merge_type == :heading) ||
            (node.respond_to?(:type) && node.type.to_s == "heading")
        end
    ...
    context "when refiner returns nodes not in statement lists" do
      ...
      it "skips match when template_node is not found" do
        ...
        allow(fake_node).to receive_messages(
          type: :paragraph,
          source_position: {start_line: 999, end_line: 999},
        )
      ...
      it "skips match when dest_node is not found" do
        ...
        allow(fake_node).to receive_messages(
          type: :paragraph,
          source_position: {start_line: 888, end_line: 888},
        )
